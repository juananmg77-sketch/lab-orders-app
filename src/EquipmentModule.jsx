...
import { Microscope, Activity, Settings, Plus, FileText, Trash2, Eye, ShieldCheck, AlertTriangle, ChevronLeft, Upload, Target, Thermometer, Scale, Download, Database, Users, RefreshCcw, Waves, Box, FlaskConical, Zap, Cpu, Sun, Wind } from 'lucide-react';
...
export default function EquipmentModule({ session, onLogout, globalLab, onBackToHub, role = 'operations' }) {
  ...
  const filteredEquipments = equipments.filter(eq => {
    ...
    return nameStr.includes(term) || codeStr.includes(term) || modelStr.includes(term);
  });
            ...
            <div className="flex-between" style={{ marginBottom: '24px' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  {selectedCategory && (
                    <button 
                      onClick={() => { setSelectedCategory(null); setSelectedSubtype('Todos'); setSelectedEquipments([]); }}
              ...
              <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
                <div style={{ position: 'relative' }}>
                  <input 
                    type="text" 
                    placeholder="Filtrar por nombre, modelo o SN..." 
                ...
                <button 
                  ...
                  onClick={() => {
                    const codes = equipments
                      .map(e => {
                        const match = (e.equipment_code || '').match(/EQ-(\d+)/);
                        return match ? parseInt(match[1]) : 0;
                      })
                      .filter(n => !isNaN(n));
                    
                    const nextNum = codes.length > 0 ? Math.max(...codes) + 1 : 1;
                    const nextCode = `EQ-${String(nextNum).padStart(3, '0')}`;
                ...
                <div className="card table-wrapper">
                  ...
                  <tbody>
                    {tableEquipments.map(eq => {
                      ...
                      return (
                        <tr key={eq.id} style={{ opacity: isBaja ? 0.6 : 1, backgroundColor: selectedEquipments.includes(eq.id) ? 'var(--primary-light)' : 'transparent' }}>
                          ...
                          <td style={{ fontWeight: 600, fontSize: '0.9rem' }}>
                            {eq.cal_valid_until ? new Date(eq.cal_valid_until).toLocaleDateString() : 'N/A'}
                          </td>
                          <td style={{ textAlign: 'center' }}>
                            <div style={{ display: 'flex', gap: '8px', justifyContent: 'center' }}>
                              <button 
                                className="btn btn-secondary" 
                                style={{ padding: '6px', fontSize: '0.75rem', color: 'var(--primary)', border: '1px solid var(--border)' }}
