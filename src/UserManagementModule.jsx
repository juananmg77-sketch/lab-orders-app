...
export default function UserManagementModule({ onBackToHub }) {
  ...
  const [newUser, setNewUser] = useState({ email: '', password: '', role: 'operations', delegacion: 'Baleares' });
  ...
  const handleCreateUser = async (e) => {
    ...
    if (data.user) {
      ...
      if (profileError) {
        ...
        setIsAdding(false);
        setNewUser({ email: '', password: '', role: 'operations', delegacion: 'Baleares' });
        fetchUsers();
      }
    }
    setLoading(false);
  ...
  const handleUpdateDelegacion = async (userId, newDelegacion) => {
    const { error: updateError } = await supabase
      .from('profiles')
      .update({ delegacion: newDelegacion })
      .eq('id', userId);

    if (updateError) {
      alert("Error actualizando delegación: " + updateError.message);
    } else {
      fetchUsers();
    }
  };
          ...
          <div style={{ backgroundColor: '#fff1f2', border: '1px solid #fda4af', padding: '20px', borderRadius: '12px', marginBottom: '30px', color: '#991b1b' }}>
            ...
            <div style={{ marginTop: '16px', backgroundColor: '#0f172a', color: '#94a3b8', padding: '16px', borderRadius: '8px', fontSize: '0.85rem' }}>
              <code>
                ...
                &nbsp;&nbsp;delegacion text DEFAULT 'Baleares',<br/>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead style={{ backgroundColor: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
              <tr>
                ...
                <th style={{ padding: '16px 24px', textAlign: 'left', fontWeight: 600, color: '#64748b' }}>Delegación</th>
                <th style={{ padding: '16px 24px', textAlign: 'left', fontWeight: 600, color: '#64748b' }}>Laboratorio</th>
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  ...
                  <td style={{ padding: '16px 24px' }}>
                    <select
                      value={u.delegacion || 'Baleares'}
                      onChange={(e) => handleUpdateDelegacion(u.id, e.target.value)}
                  ...
                  <td style={{ padding: '16px 24px' }}>
                    <span style={{
                      ...
                      backgroundColor: (u.delegacion || 'Baleares') === 'Canarias' ? '#f0fdf4' : '#eff6ff',
                      color: (u.delegacion || 'Baleares') === 'Canarias' ? '#166534' : 'var(--primary)'
                    }}>
                      {labFromDelegacion(u.delegacion || 'Baleares')}
                    </span>
                  </td>
                  <td style={{ padding: '16px 24px' }}>
                    <select
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }}>
          <div className="card" style={{ width: '400px', padding: '32px' }}>
            ...
            <form onSubmit={handleCreateUser}>
              ...
              <div className="input-group">
                <label className="input-label">Delegación</label>
                <select className="input-field" value={newUser.delegacion} onChange={e => setNewUser({...newUser, delegacion: e.target.value})}>
                  {DELEGACIONES.map(d => <option key={d} value={d}>{d}</option>)}
                </select>
                <p style={{ margin: '4px 0 0', fontSize: '0.8rem', color: '#64748b' }}>
                  Laboratorio asignado: <b>{labFromDelegacion(newUser.delegacion)}</b>
                </p>
              </div>
              <div className="input-group">
