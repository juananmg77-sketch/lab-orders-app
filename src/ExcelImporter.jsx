...
export default function ExcelImporter({ isOpen, onClose, existingArticles, onImportDone, selectedLab = 'HSLAB Baleares' }) {
  ...
  const executeImport = async () => {
    ...
    try {
      ...
      // 2. Ensure they exist in the database for this lab
      for (const supName of fileSuppliers) {
        const { data: existingSup } = await supabase.from('suppliers').select('id').eq('name', supName).eq('lab', selectedLab).single();
        if (!existingSup) {
          await supabase.from('suppliers').insert({ name: supName, lab: selectedLab });
          supCount++;
        }
      }

      // 3. Process Articles — only match existing articles within the same lab
      const labExistingArticles = existingArticles.filter(a => (a.lab || 'HSLAB Baleares') === selectedLab);
      ...
      for (const row of validRows) {
        ...
        // Only match articles within the same lab to allow cross-lab duplicates
        const existing = labExistingArticles.find(a =>
        ...
        if (existing) {
          ...
          // Generate a unique ID; if a row ID is provided, only use it if not already taken in any lab
          const rowId = row[mappings.id] ? String(row[mappings.id]).trim() : null;
          const idTaken = existingArticles.some(a => a.id === rowId);
          const newId = (rowId && !idTaken) ? rowId : `LAB-${Math.random().toString(36).substr(2, 6).toUpperCase()}`;
