...
function PurchasingModule({ session, onLogout, globalLab, onBackToHub, role = 'operations' }) {
  ...
  }, [articleSearch, articleCategory, articleSupplier, labArticles]);

  const uniqueCategories = useMemo(() => [...new Set(labArticles.filter(a => a && a.category).map(a => a.category))], [labArticles]);
  const uniqueSuppliers = useMemo(() => {
    const allNames = [
      ...labArticles.filter(a => a && a.supplierName).map(a => a.supplierName),
  ...
  const handleSyncSuppliers = async () => {
    ...
    const existingLower = new Set(labSuppliers.map(s => s.name.toLowerCase()));
        ...
        <header className="header">
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <span style={{
              ...
              {selectedLab === 'HSLAB Canarias' ? '🌴 Canarias' : '🏝️ Baleares'}
            </span>
          </div>
